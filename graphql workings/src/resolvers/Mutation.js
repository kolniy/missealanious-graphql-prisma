...
const Mutation = {
    ...
    updatePost(parent, { id, data }, { db, pubsub }, info){
        ...
        if(typeof data.published === 'boolean'){
            ...
            } else if (!originalPost.published && post.published) // this condition checks if the post has not being published and now needs to be publsihed, 
            // so the created event should run and the post can be displayed by the UI.
             {
             pubsub.publish("post", {
               post: {
                 mutation: "CREATED",
        ...
        } else if (post.published) // this condition simply checks if the post was already published and the some of the post
        // properties were update, so the update event can be fired and the UI responds accordingly.
        {  
             pubsub.publish("post", {
               post: {
                 mutation: "UPDATED",
